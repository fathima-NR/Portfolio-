function TelehealthMock() {
  return (
    <div className="overflow-hidden rounded-2xl bg-bg p-3 text-[11px] ring-1 ring-line">
      <div className="mb-3 flex items-center justify-between px-1 text-[10px] font-bold uppercase tracking-[0.14em] text-muted">
        <span>Agent portal</span>
        <span className="rounded-full bg-accent px-2 py-0.5 text-white">Live</span>
      </div>
      <div className="grid grid-cols-[88px_1fr] overflow-hidden rounded-xl bg-white">
        <aside className="space-y-1 border-r border-line p-3 text-muted">
          {["Leads", "Calls", "Nurses", "Admin"].map((item, index) => (
            <div
              key={item}
              className={`rounded-lg px-2 py-1.5 ${index === 1 ? "bg-accent font-bold text-white" : ""}`}
            >
              {item}
            </div>
          ))}
        ...
        <div className="space-y-2 p-3">
          ...
          {["AWV eligibility · Medicare", "CHI follow-up assigned", "SDOH screening queued"].map((row) => (
            <div key={row} className="rounded-lg bg-bg px-3 py-2 text-muted">
              {row}
            </div>
          ))}
        </div>
...
function HrMock() {
  return (
    <div className="rounded-2xl bg-bg p-4 text-[12px] ring-1 ring-line">
      ...
      <div className="space-y-2">
        {[
          ...
          <div key={name} className="grid grid-cols-[1fr_auto_auto] gap-3 rounded-lg bg-white px-3 py-2">
            <span>{name}</span>
...
function SitesMock() {
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      {["medcare.org", "careconnexhealth.com"].map((site) => (
        <div key={site} className="rounded-2xl bg-bg p-4 ring-1 ring-line">
